...
class Scene(Serializable):
    ...
    def resetLastSelectedStates(self):
        """
        先选中节点1，后选中节点2，需要将节点1的选中状态置为False，这个函数把所有历史选中的都清空
    ...
    def setNodeClassSelector(self, class_selecting_function: 'functon') -> 'Node class type':
        """
        通过这个来设置反序列化时使用哪个Node，使用方法setNodeClassSelector(lambda data: Class)，所以这里理应有一个函数实现获取不同类型的NodeClass，参考getNodeClassFromData
    ...
    def deserialize(self, data: dict, hashmap: dict={}, restore_id: bool=True, *args, **kwargs) -> bool:
        # self.clear()      # 相比clear后重建全部，现在的实现效率更高，对undo/redo也更友好
        hashmap = {}
