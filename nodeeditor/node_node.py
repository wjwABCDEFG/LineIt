# -*- coding: utf-8 -*-
"""
节点类，属性包括title，inputs/outputs连接点，以及功能相关
...
class Node(Serializable):
    ...
    def __init__(self, scene: 'Scene', title: str="Undefined Node", inputs: list=[], outputs: list=[]):
        """
        这里要把scene传过来的原因是作者的小设计，就是原本的逻辑是
        node = Node()
        scene.addNode(node)
        但作者觉得这样不够优雅，最好新建node的时候就默认加进场景中，语句如下
        node = Node(scene)
        这样就优雅很多，即不需要手动addNode，而是在Node的构造函数中完成了add逻辑
    ...
    def remove(self):
        """
        删除画布中的node，删除node中所有socket连接的线条，图元本身置None，删除画布scene中的记录
    ...
    def onMarkedInvalid(self):
        ...
        # 暴露给后续的扩展，如果子类需要在标记为invalid或者dirty的时候触发一些操作可以重写这个
        pass
    ...
    def getChildrenNodes(self) -> 'List[Node]':
        """
        这里只找一层的子节点，需要查找所有子节点需要递归，参考markDescendantsDirty
    ...
    def getInput(self, index: int=0) -> ['Node', None]:
        ...
        注意这个方法一般只针对输入端只能连接一条线的情况，如果节点编辑器被设置成一个输入端可以被多条线连接，则需要使用getInputs方法
            ...
            for socket_data in data['inputs']:
                ...
                if found is None:
                    ...
                    found = self.__class__.Socket_class(
                        ...
                        socket_type=socket_data['socket_type'], multi_edges=socket_data['multi_edges'],
