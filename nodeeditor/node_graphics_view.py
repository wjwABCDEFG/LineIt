# -*- coding: utf-8 -*-
"""
背景画布绘制模式（抗锯齿，全量更新）和按键操作事件（滚轮放大缩小，中键画布拖拽）
...
from PySide6.QtGui import QPainter, QMouseEvent, QWheelEvent, QKeyEvent, QDragEnterEvent, QDropEvent
...
class QDMGraphicsView(QGraphicsView):
    ...
    def __init__(self, grScene: 'QDMGraphicsScene', parent: 'QWidget'=None):
        ...
        self.setScene(self.grScene)     # QGraphicsView需要绑定一个scene，不要搞反了，QGraphicsView只是设置背景，所有操作在这里
        ...
        self.zoomInFactor = 1.25    # 缩放大小（真正控制滚轮一次缩放的程度）
        self.zoomClamp = True
        self.zoom = 5               # 当前的缩放值（记录当前是多少等级，这是为了用户方便记录的虚拟的值）
        self.zoomStep = 1           # 次数（记录当前是多少等级，这是为了用户方便记录的虚拟的值）
        self.zoomRange = [0, 10]    # 允许缩放的范围（记录当前是多少等级，这是为了用户方便记录的虚拟的值）
    ...
    #         item = self.snapping.getSnappedSocketItem(event)
        ...
        if item is None:
            if event.modifiers() & Qt.ControlModifier:
                ...
                # 假事件：鼠标释放事件，左键，变为无按键，但这里感觉不用假事件也可以
    ...
    #             self.mode = MODE_NOOP
            ...
            if self.rubberBandDraggingRectangle:
                ...
                if current_selected_items != self.grScene.scene._last_selected_items:   # 点击的物体有变
    ...
    def cutIntersectingEdges(self):
        """这段其实非常耗，鼠标移动过程中会一直收集移动到的点，第一层for先把points[i]->points[i+1]连成线，再内层for遍历场景中的线与其相交"""
    ...
    def getItemAtClick(self, event: QEvent) -> 'QGraphicsItem':
        ...
        """
        pos = event.pos()
        obj = self.itemAt(pos)  # 这个api可以判断点击位置是什么图元QGraphicsItem
        return obj
