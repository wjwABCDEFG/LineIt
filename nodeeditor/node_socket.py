...
"""
连接点类，属性包括position
socket_type是颜色，对应的是SOCKET_COLORS的表
...
class Socket(Serializable):
    ...
    def __init__(self, node: 'Node', index: int=0, position: int=LEFT_TOP, socket_type: int=1, multi_edges: bool=True,
        ...
        :param socket_type: 颜色，Constant defining type(color) of this socket
    ...
    def __str__(self):
        return "<Socket #%d %s %s..%s -- Edges: %s>" % (
            self.index, "ME" if self.is_multi_edges else "SE", hex(id(self))[2:5], hex(id(self))[-3:], self.edges
        )
