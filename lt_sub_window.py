...
from lt_conf import get_class_from_opcode, LINEIT_NODES, LISTBOX_MIMETYPE
...
class LineItSubWindow(NodeEditorWidget):
    def __init__(self):
        super().__init__()
    ...
    def doEvalOutputs(self):
        ...
        for node in self.scene.nodes:
            if node.__class__.__name__ == "LineItNode_Output":
                node.eval()
    ...
    def closeEvent(self, event):    # 有很多类似这样的设计，是为了当一个子窗口关闭时，检测所有子窗口的关闭事件？还是说仅仅是为了让具体的执行操作写在上层并支持一个队列
    ...
    def contextMenuEvent(self, event):
        try:
            ...
            if type(item) == QGraphicsProxyWidget:
                item = item.widget()        # 注意这里QGraphicsItem对象获取widget对象
    ...
    def handleNodeContextMenu(self, event):
        ...
        context_menu = QMenu(self)      # 除了MainWindow自带的menubar，右键菜单需要使用QMenu创建
        markDirtyAct = context_menu.addAction("Mark Dirty")
        markDirtyDescendantsAct = context_menu.addAction("Mark Descendant Dirty")
        markInvalidAct = context_menu.addAction("Mark Invalid")
        unmarkInvalidAct = context_menu.addAction("Unmark Invalid")
        evalAct = context_menu.addAction("Eval")
        action = context_menu.exec_(self.mapToGlobal(event.pos()))  # 显示menu，或者可以使用event.globalPos()
    ...
    def handleNewNodeContextMenu(self, event):
        ...
        if action is not None:
            ...
            if self.scene.getView().mode == MODE_EDGE_DRAG:
                # 这种写法实际上是当鼠标正在左键拖出一条线，未释放时是MODE_EDGE_DRAG，此时再按下右键，有点反人类，可以把这段逻辑移到鼠标左键释放时如果itemAt不是socket，则弹出该menu
