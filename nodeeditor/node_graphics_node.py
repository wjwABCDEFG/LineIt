...
from PySide6.QtWidgets import QGraphicsItem, QGraphicsTextItem, QWidget, QGraphicsProxyWidget
...
class QDMGraphicsNode(QGraphicsItem):
    ...
    def __init__(self, node: 'Node', parent: QWidget = None):
        ...
        # init our flags
        self.hovered = False
        self._was_moved = False     # 这个标志位是为了表达记录日志的时机，“拖动完释放鼠标”才记录，光拖动不释放和光释放不拖动都不会记录
    ...
    def initAssets(self):
        ...
        self._pen_default.setWidthF(self.pen_width)
    ...
    def doSelect(self, new_state=True):
        ...
        :type new_state: ``bool``
        """
        self.setSelected(new_state)     # qt gui变化
        self._last_selected_state = new_state   # 状态变化
        if new_state: self.onSelected()     # emit signal相关事件出发
    ...
    def initContent(self):
        ...
        # self.grContent = QGraphicsProxyWidget(self)   # QGraphicsProxyWidget相当于一个空的widget
        # self.grContent.setWidget(self.content)
        if self.content is not None:
            # setGeometry相当于给挂接的widget一个范围框
    ...
    def paint(self, painter, QStyleOptionGraphicsItem, widget=None):
        ...
        path_content.addRoundedRect(0, self.title_height, self.width, self.height - self.title_height + self.node.content.height, self.edge_roundness, self.edge_roundness)
        ...
        path_outline.addRoundedRect(-1, -1, self.width + self.pen_width, self.height + self.pen_width + self.node.content.height, self.edge_roundness, self.edge_roundness)
